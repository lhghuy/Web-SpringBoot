...
class _AdminInvoiceScreenState extends State<AdminInvoiceScreen> {
  ...
  String? error;
  Map<String,
      dynamic>? invoice; // expected structure: { order: {...}, orderDetails: [...], total: 0 }
  ...
  void didChangeDependencies() {
    ...
    if (args is Map && args['invoice'] != null) {
      ...
      if (idx >= 0 && idx + 1 < parts.length &&
          parts[idx + 1].isNotEmpty) { // Thêm kiểm tra empty string
  ...
  Future<void> _loadInvoice(String orderId) async {
    ...
    try {
      final r = await api.get(
          '/admin/orders/$orderId/bill'); // r is already Map<String, dynamic>

      // SỬA LỖI: Thay đổi cách xử lý phản hồi từ API
      // Hỗ trợ các cấu trúc:
      // 1. { "success": true, "data": { ...invoice_data... } }
      // 2. { "data": { ...invoice_data... } }
      // 3. { ...invoice_data... } (dữ liệu hóa đơn trực tiếp)
      Map<String, dynamic> effectiveInvoiceData;

      if (r.containsKey('success') && r['success'] == true &&
          r.containsKey('data') && r['data'] is Map) {
        effectiveInvoiceData = r['data'] as Map<String, dynamic>;
      } else if (r.containsKey('data') && r['data'] is Map) {
        effectiveInvoiceData = r['data'] as Map<String, dynamic>;
      } else if (r.containsKey('order') && r.containsKey('orderDetails') &&
          r.containsKey('total')) {
        // Giả định đây là các trường chính trong dữ liệu hóa đơn trực tiếp
        effectiveInvoiceData = r;
      } else {
        throw Exception('Cấu trúc dữ liệu hóa đơn không hợp lệ từ API.');
      }

      invoice = Map<String, dynamic>.from(effectiveInvoiceData);
    } catch (e) {
      error = e.toString();
    } finally {
  ...
  String fmtCurrency(dynamic amount) {
    ...
    if (amount is num)
      ...
      v = num.tryParse(amount.toString()) ?? 0;
  ...
  String fmtDateTime(String? iso) {
    ...
    try {
      ...
      return DateFormat('dd/MM/yyyy HH:mm').format(
          dt.toLocal()); // Thêm .toLocal() để hiển thị đúng múi giờ
    } catch (_) {
      return iso;
    }
  }
  ...
  Widget _buildHeader() {
    ...
    return Column(
      children: [
        ...
        Text('HÓA ĐƠN THANH TOÁN', style: Theme
            .of(context)
            .textTheme
            .titleLarge
            ?.copyWith(color: Colors.blue, fontWeight: FontWeight.bold)),
        // SỬA LỖI: headline6 -> titleLarge
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            // SỬA LỖI: Kiểm tra null và tableNumber một lần cho gọn
            _buildInfoRow(
                'Bàn', order?['table']?['tableNumber']?.toString() ?? 'N/A'),
            _buildInfoRow(
                'Nhân viên', order?['createdBy']?['fullName'] ?? 'N/A'),
            _buildInfoRow(
                'Thời gian', fmtDateTime(order?['createdAt']?.toString())),
          ],
        ),
      ),
    );
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: const [
                Expanded(flex: 4,
                    child: Text('Tên món', style: TextStyle(
                ...
                Expanded(flex: 3,
                    child: Text('Thành tiền', style: TextStyle(
                        color: Colors.white, fontWeight: FontWeight.bold),
                        textAlign: TextAlign.right)),
              ],
            ),
          ...
          ...items.map((it) {
            final name = it['food']?['name'] ?? it['foodItem']?['name'] ??
                it['name'] ?? '-';
            final qty = it['quantity'] ?? it['qty'] ?? 0;
            final price = it['priceAtOrderTime'] ?? it['price'] ?? 0;
            final line = (price is num ? price : num.tryParse(
                price.toString()) ?? 0) *
                ...
                children: [
                  ...
                  Expanded(flex: 3,
                      child: Align(alignment: Alignment.centerRight,
                          child: Text(fmtCurrency(price)))),
                  Expanded(flex: 3,
                      child: Align(alignment: Alignment.centerRight,
                          child: Text(fmtCurrency(line)))),
                ],
              ),
            );
          }).toList(),
          ...
          Container(
            ...
            child: Row(
              children: [
                const Expanded(flex: 9,
                    child: Text('TỔNG TIỀN', style: TextStyle(
                        color: Colors.white, fontWeight: FontWeight.bold))),
                Expanded(flex: 3,
                    child: Align(alignment: Alignment.centerRight,
                        child: Text(fmtCurrency(total), style: const TextStyle(
                            color: Colors.white,
